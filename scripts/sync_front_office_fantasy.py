#!/usr/bin/env python3
"""Build the authoritative Front Office Fantasy dynasty snapshot from Sleeper."""
...
DEFAULT_CONFIG = ROOT / "config" / "front-office-fantasy.json"
DEFAULT_OUTPUT = ROOT / "data" / "front-office-fantasy-master.json"
...
def fetch_json(path: str) -> Any:
    request = urllib.request.Request(
        f"{API_ROOT}{path}", headers={"User-Agent": "Front-Office-Fantasy-Sync/1.0"}
    )
    try:
...
def build_snapshot(config: dict[str, Any], raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(league.get("league_id")) != league_id:
        raise SyncError("Sleeper response did not match the configured Front Office Fantasy league ID")
    if clean_name(league.get("name")) != clean_name(config["league_name"]):
        raise SyncError("Sleeper response did not match the configured Front Office Fantasy league name")
    if str(league.get("season")) != str(config["season"]):
        raise SyncError("Sleeper response did not match the configured Front Office Fantasy season")
    ...
    if len(raw["rosters"]) != int(config["expected_team_count"]):
        raise SyncError(
            f"Expected {config['expected_team_count']} rosters; received {len(raw['rosters'])}"
        )
    ...
    for roster in sorted(raw["rosters"], key=lambda item: int(item["roster_id"])):
        ...
        if not user:
            raise SyncError(f"Roster {roster['roster_id']} has no matching Front Office Fantasy user")
...
def team_report(snapshot: dict[str, Any], configured_name: str) -> str:
    ...
    lines = ["# TJS2025 Front Office Fantasy Team Report", ""]
    if not team:
        return "\n".join(lines + ["The configured user team was not found in current Sleeper data."])
...
def league_report(snapshot: dict[str, Any]) -> str:
    ...
    lines = [
        "# Front Office Fantasy League Snapshot",
        "",
...
def sync(config_path: Path, output_path: Path, check_only: bool = False) -> dict[str, Any]:
    ...
    if old_fingerprint == fingerprint:
        print(f"Front Office Fantasy data unchanged ({fingerprint[:12]})")
        return old
    now = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    snapshot["sync"] = {"last_changed_at": now, "source_fingerprint": fingerprint}
    if check_only:
        print(f"Front Office Fantasy data valid; changes detected ({fingerprint[:12]})")
        return snapshot
    if old:
    ...
    print(f"Front Office Fantasy data refreshed ({fingerprint[:12]})")
    return snapshot
